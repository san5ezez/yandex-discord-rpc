...
exports.ClientUser = exports.StatusDisplayType = exports.ActivityPartyPrivacy = exports.ActivitySupportedPlatform = void 0;
...
class ClientUser extends User_1.User {
    ...
    async setActivity(activity, pid) {
        const formattedActivity = {
            name: activity.name,
            type: activity.type ?? v10_1.ActivityType.Playing,
            created_at: Date.now(),
            instance: !!activity.instance
        };
        // URL only for Streaming activity
        if (activity.type === v10_1.ActivityType.Streaming && activity.url) {
        ...
        if (activity.state)
            formattedActivity.state = activity.state;
        if (activity.detailsUrl)
            formattedActivity.details_url = activity.detailsUrl;
        if (activity.stateUrl)
            formattedActivity.state_url = activity.stateUrl;
        ...
        if (activity.largeImageKey ||
            ...
            if (activity.largeImageKey)
                formattedActivity.assets.large_image = activity.largeImageKey;
            if (activity.smallImageKey)
                formattedActivity.assets.small_image = activity.smallImageKey;
            if (activity.largeImageText)
                formattedActivity.assets.large_text = activity.largeImageText;
            if (activity.smallImageText)
                formattedActivity.assets.small_text = activity.smallImageText;
            if (activity.largeImageUrl)
                formattedActivity.assets.large_url = activity.largeImageUrl;
            if (activity.smallImageUrl)
                formattedActivity.assets.small_url = activity.smallImageUrl;
        }
        // Status display type
        ...
        if (activity.partyId || activity.partySize || activity.partyMax) {
            ...
            if (activity.partyId)
                formattedActivity.party.id = activity.partyId;
        ...
        if (activity.joinSecret || activity.spectateSecret || activity.matchSecret) {
            ...
            if (activity.joinSecret)
                formattedActivity.secrets.join = activity.joinSecret;
            if (activity.spectateSecret)
                formattedActivity.secrets.spectate = activity.spectateSecret;
            if (activity.matchSecret)
                formattedActivity.secrets.match = activity.matchSecret;
        }
        // Buttons
